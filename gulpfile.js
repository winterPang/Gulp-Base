...
gulp.task('clean', function () {
    return gulp.src('./oasis', {read: false})
        .pipe(plugins.clean());
});

// 起始目录和目标目录，据说只支持相对路径，不过也够用了 
gulp.task('copyChanged', function () {
    gulp.src(['./src/**/*.*', '!./src/init/**/*.json', '!./src/**/*.less'])
        .pipe(plugins.plumber())
...
gulp.task('copyInitData', function () {
    gulp.src('./doc/**/*')
        .pipe(plugins.plumber())
        .pipe(plugins.changed('./oasis/doc'))
        .pipe(gulp.dest('./oasis/doc'));
...
gulp.task('watch', function () {
    ...
    gulp.watch('src/**/*.js', function () {       //当js文件变化后，自动检验 压缩  
        console.log("change") ;                                           //gulp.run('lint', 'scripts');
        gulp.run('lint', 'es6');
    });

    //监听less变化 
    gulp.watch(['src/**/*.less', '!src/frame/libs'], function () {
        gulp.run('less', 'es6', 'copyChanged');
    });
    ...
    //监听假数据变化  
    gulp.watch('src/init/**/*.json', ['copyInitData']);

})

gulp.task('default', [/*'clean',*/'connect', 'watch', 'copyChanged', 'copyInitData', 'less', 'es6'], function () {
    require('opn')(
